...
# Default network URL
default_network_url="http://127.0.0.1:8080"
...
for template in "${templates[@]}"; do
  ...
  kda gen -t "$template" -d devnet.yaml -o "$yaml_file"

  # Sign the YAML file with dev.key
  kda sign -k dev.key "$yaml_file"
  kda sign -k user.key "$yaml_file"
  # Sign the YAML file again with admin.key, expecting it to become a JSON file
  kda sign -k admin.key "$yaml_file"
