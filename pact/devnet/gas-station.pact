...
(module gas-station GOVERNANCE
  (defcap GOVERNANCE ()
    (enforce-guard (keyset-ref-guard "dab.bonder-admin")))

  (implements gas-payer-v1)
  (use coin)
  (use dab.gas-guards)

  (defconst GAS_STATION:string "cab-free-gas")
  ...
  (defcap GAS_PAYER:bool
    ...
    (let
        ( (enforce-ns (lambda (i) (enforce (= "(dab." (take 5 (at i (at "exec-code" (read-msg))))) "only dab namespace on top level")))
