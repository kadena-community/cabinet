...
(module poller GOVERNANCE
  ...
  (use dab.bonder [add-new-poll read-bond read-lockup get-lockup-key can-account-bond add-user-interaction])
...
(if (read-msg 'upgrade)
    ["upgrade"]
    [
     (dab.bonder.grant-privilege (bonder-guard) "aggregate-interaction")
